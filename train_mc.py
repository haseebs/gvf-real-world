...
data = h5py.File('data-logs/processed_data.hdf5', 'r')
...
for epoch in range(args['epochs']):
    ...
    _, _, train_error_avg = trainers.train_epoch_using_return(model, train_dataloaders[0], optimizer, criterion, args,
                                                              log_every=-1)
    ...
    if epoch % 500 == 0:
        ...
        train_preds, _, _ = trainers.train_epoch_using_return(evaluation_model, train_dataloaders_non_shuffled[0],
    ...
    if epoch % 1000 == 0 and epoch != 0:
        ...
        for test_idx, test_dataloader in enumerate(test_dataloaders):
            ...
            test_nmse_score = utils.compute_scores(returns[test_data[test_idx].starting_idx + 1: test_data[test_idx].ending_idx],
                                                   test_preds[:-1],
                                                   test_data[test_idx].return_variance)
            ...
            for step in range(len(test_preds)):
                ...
                test_predictions_logger.add_data([exp.run, epoch, test_idx, step, test_preds[step].item(),
                                                  float(returns[test_data[test_idx].starting_idx + step + 1]),
                                                  float(cummulants[test_data[test_idx].starting_idx + step])])

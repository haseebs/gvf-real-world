...
path_to_model = f"saved_models/{args['name']}/{args['sensor']}/lr_{args['lr_offline']}"
...
model = NLinear(train_data[0][0].shape[0], args['lookback_window_size']+1, cummulant_idx, independent=True).to(args['device']) # +1 because it doesnt count current state
...
for epoch in range(args['epochs']):
    ...
    if epoch % 500 == 0:
        ...
        torch.save({
                    ...
                    }, path_to_model + "_epoch" + str(epoch) + ".pt")
...
summary_logger.add_data([exp.run, epoch, train_error_avg, eval_error_avg, test_error_avg, train_nmse_score, eval_nmse_score,

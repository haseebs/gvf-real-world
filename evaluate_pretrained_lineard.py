...
model = NLinear(val_data[0][0].shape[1], args['lookback_window_size']+1, cummulant_idx, independent=True).to(args['device']) # +1 because it doesnt count current state
...
# load saved model
path_to_model = f"saved_models/{args['saved_model_name']}/{args['sensor']}/lr_{args['lr_offline']}_epoch{args['epochs']}.pt"
...
eval_preds, _, eval_error_avg = trainers.train_epoch_nstep_online(evaluation_model, val_dataloaders[0],
                                                            evaluation_optim, criterion, args, frozen=False,
                                                            log_every=-1, nstep_n=args['prediction_horizon'])
eval_nmse_score = utils.compute_scores(returns[val_data.starting_idx: val_data.ending_idx],
                                        eval_preds,
...
for test_idx, test_dataloader in enumerate(test_dataloaders):
    ...
    for param_group in evaluation_optim.param_groups:
        param_group['lr'] = args['lr_online']
    evaluation_model.train()
    test_preds, test_errors, test_error_avg = trainers.train_epoch_nstep_online(evaluation_model, test_dataloader,
                                                                                evaluation_optim,
                                                                                criterion, args, frozen=False,
                                                                                log_every=1,
                                                                                nstep_n=args['prediction_horizon'])
    test_nmse_score = utils.compute_scores(returns[test_dataloader.dataset.starting_idx:test_dataloader.dataset.ending_idx],
                                           test_preds,
                                           test_dataloader.dataset.return_variance)
    print("Test NMSE: ", test_nmse_score)
    test_nmse_logger.add_data([exp.run, args['epochs'], test_idx, test_nmse_score])
    test_nmse_scores.append(test_nmse_score)
    for step in range(len(test_preds)):
        #test_error_logger.add_data([exp.run, epoch, test_idx, step, test_errors[step]]) #bugged for now
        test_predictions_logger.add_data([exp.run, args['epochs'], test_idx, step, test_preds[step].item(),
